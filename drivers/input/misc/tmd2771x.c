...
static void tmd2771x_power(struct i2c_client *client, int enable);
...
static int tmd2771x_parse_dt(struct i2c_client *client, struct tmd2771x_platform_data *pdata)
{
	int err = 0;
	/* TODO dts */
        pdata->irq = gpio_to_irq(TMD2771X_GPIO_INT);
        pdata->ps_det_thld = 40;
        pdata->ps_hsyt_thld = 30;
        pdata->als_hsyt_thld = 20;

	err = gpio_tlmm_config(GPIO_CFG(TMD2771X_GPIO_INT, 0,GPIO_CFG_INPUT, GPIO_CFG_PULL_UP,
	                       GPIO_CFG_8MA), GPIO_CFG_ENABLE);
	gpio_request(TMD2771X_GPIO_INT, "tmd2711x_gpio_int");
	gpio_direction_input(TMD2771X_GPIO_INT);
	return 0;
}
...
static void save_tmd_caldata(struct work_struct *work)
{
    ...
    write_file("/system/tmd_cal.txt", temp, 64);
...
static s32 tmdcal_write_proc(struct file *filp, const char __user *buff, unsigned long len, void *data)
{
    char flag[64];
    int ret =0;
...
static s32 tmdcal_read_proc( char *page, char **start, off_t off, int count, int *eof, void *rdata )
{
    int n = 0;

    n = sprintf(page, "thld:%d, max:%d, min:%d\n", data->ps_det_thld, pdata_max, pdata_min);
				   ...
				   const struct i2c_device_id *id)
{
	...
	struct proc_dir_entry *tmdcal_proc_file =
		create_proc_entry("tmd_cal", 0666, NULL);

	if (tmdcal_proc_file) {
		tmdcal_proc_file->read_proc = tmdcal_read_proc;
		tmdcal_proc_file->write_proc = tmdcal_write_proc;
	} else
