...
/*
 dfsfds* This software program is licensed subject to the GNU General Public License
...
//puyan add bma222 and bma222e compatible
#define BMA222_ADDRESS			0x18 //0x08
...
static void bma222_get_power(struct i2c_client *client);
static void bma222_power(struct i2c_client *client, int enable);
struct regulator *bma_vdd = NULL;
struct regulator *bma_vio = NULL;
...
static void bma222_work_func(struct work_struct *work)
{
	...
	{
		input_report_abs(bma222->input, ABS_X, acc.y);//puyan modify x value
		input_report_abs(bma222->input, ABS_Y, acc.x*(-1));		
		input_report_abs(bma222->input, ABS_Z, acc.z);
	}
#else
	{
	
		input_report_abs(bma222->input, ABS_X, acc.x);//puyan modify x value
	...
	//printk("^^^^^^^^%s^^^^^^^^ bma222_data:x:%d,y:%d,z:%d\n",__func__,acc.x,acc.y,-acc.z);
	bma222->value = acc;
...
static void bma222_get_power(struct i2c_client *client)
{
    bma_vdd = devm_regulator_get(&client->dev, "vdd");
    if(bma_vdd == NULL){
	goto VIO_GET;
    }
    regulator_set_voltage(bma_vdd, 2850000, 2850000);

VIO_GET:
    bma_vio = devm_regulator_get(&client->dev, "vio");
    if(bma_vio == NULL) {
	printk("Error of setting voltage\n");
        return;
    }
    regulator_set_voltage(bma_vio, 1800000, 1800000);
    mdelay(100);
    printk("^^^^^^^^^^^^^^^^bma222 get power ok!\n");
}

static void bma222_put_power(struct i2c_client *client)
{
    if(bma_vdd){
	devm_regulator_put(bma_vdd);
    }
    if(bma_vio){
	devm_regulator_put(bma_vio);
    }
}
...
static void bma222_power(struct i2c_client *client, int enable)
{
    ...
    if(enable) {
	if(bma_vdd){
	    err = regulator_enable(bma_vdd);
	}
	if(bma_vio){
            err = regulator_enable(bma_vio);
	}
        mdelay(100);
    } else {
	if(bma_vdd){
		...
		const struct i2c_device_id *id)
{
	...
	/* read chip id */
	tempvalue = 0;
	bma222_get_power(client);
	bma222_power(client,1);
	tempvalue = i2c_smbus_read_word_data(client, BMA222_CHIP_ID_REG);
	if ((tempvalue&0x00FF) == BMA222E_CHIP_ID) {
		printk(KERN_INFO "Bosch Sensortec Device detected!\n" \
				"BMA222 registered I2C driver BMA222E_CHIP_ID=0xf8!\n");
	}
	else{
		client->addr = BMA222_ADDRESS;
		tempvalue = i2c_smbus_read_word_data(client, BMA222_CHIP_ID_REG);
		if ((tempvalue&0x00FF) == BMA222_CHIP_ID) {
			printk(KERN_INFO "Bosch Sensortec Device detected!\n" \
					"BMA222 registered I2C driver BMA222_CHIP_ID=0x03!\n");
		}
		else{
...
#endif
#ifdef CONFIG_DEVICE_VERSION
	confirm_fdv(DEV_G_SENSOR,MANUF_BOSCH,MANUF_BOSCH_BMA222_ID|CHIP_ID_ADDR);
#endif
//	 bma222_power(client,0);
	return 0;
...
static struct of_device_id bma222_match_table[] = {
        { .compatible = "BMA222,bma222",},
        { },
};
...
static struct i2c_driver bma222_driver = {
	.driver = {
		.owner	= THIS_MODULE,
		.name	= SENSOR_NAME,
		.of_match_table = bma222_match_table,
	},
	.id_table	= bma222_id,
	.probe		= bma222_probe,
...
{
#ifdef CONFIG_DEVICE_VERSION
	register_fdv_with_desc(DEV_G_SENSOR,MANUF_BOSCH,MANUF_BOSCH_BMA222_ID|CHIP_ID_ADDR,DESC);
#endif
