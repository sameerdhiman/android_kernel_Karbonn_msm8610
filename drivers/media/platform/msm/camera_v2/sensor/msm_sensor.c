...
int msm_camera_fill_vreg_params_sdhi(struct camera_vreg_t *cam_vreg,
...
int msm_sensor_get_sub_module_index_sdhi(struct device_node *of_node,
...
int msm_sensor_get_dt_actuator_data_sdhi(struct device_node *of_node,
...
int msm_sensor_get_dt_csi_data_sdhi(struct device_node *of_node,
...
int msm_camera_get_dt_power_setting_data_sdhi(struct device_node *of_node,
...
int msm_camera_get_dt_gpio_req_tbl_sdhi(struct device_node *of_node,
...
int msm_camera_get_dt_gpio_set_tbl_sdhi(struct device_node *of_node,
	struct msm_camera_gpio_conf *gconf, uint16_t *gpio_array,
	uint16_t gpio_array_size)
{
	int rc = 0, i = 0;
...
int msm_camera_init_gpio_pin_tbl_sdhi(struct device_node *of_node,
	struct msm_camera_gpio_conf *gconf, uint16_t *gpio_array,
	uint16_t gpio_array_size)
{
	int rc = 0, val = 0;
...
int msm_camera_get_dt_vreg_data_sdhi(struct device_node *of_node,
	struct camera_vreg_t **cam_vreg, int *num_vreg)
{
	int rc = 0, i = 0;
	uint32_t count = 0;
...
static int msm_camera_enable_i2c_mux_sdhi(struct msm_camera_i2c_conf *i2c_conf)
{
...
static int msm_camera_disable_i2c_mux_sdhi(struct msm_camera_i2c_conf *i2c_conf)
{
	struct v4l2_subdev *i2c_mux_sd =
		dev_get_drvdata(&i2c_conf->mux_dev->dev);
	v4l2_subdev_call(i2c_mux_sd, core, ioctl,
...
int msm_camera_power_up_sdhi(struct msm_camera_power_ctrl_t *ctrl,
	...
	for (index = 0; index < ctrl->power_setting_size; index++) {
		...
		case SENSOR_I2C_MUX:
			if (ctrl->i2c_conf && ctrl->i2c_conf->use_i2c_mux)
				msm_camera_enable_i2c_mux_sdhi(ctrl->i2c_conf);
			break;
		default:
...
power_up_failed:
	...
	for (index--; index >= 0; index--) {
		...
		case SENSOR_I2C_MUX:
			if (ctrl->i2c_conf && ctrl->i2c_conf->use_i2c_mux)
				msm_camera_disable_i2c_mux_sdhi(ctrl->i2c_conf);
			break;
		default:
			pr_err("%s error power seq type %d\n", __func__,
				power_setting->seq_type);
...
msm_camera_get_power_settings_sdhi(struct msm_camera_power_ctrl_t *ctrl,
...
int msm_camera_power_down_sdhi(struct msm_camera_power_ctrl_t *ctrl,
	enum msm_camera_device_type_t device_type,
	struct msm_camera_i2c_client *sensor_i2c_client)
{
	int index = 0;
	...
	for (index = 0; index < ctrl->power_down_setting_size; index++) {
		...
		case SENSOR_CLK:
			...
			ps = msm_camera_get_power_settings_sdhi(ctrl,
						pd->seq_type,
						pd->seq_val);
			if (ps)
				msm_cam_clk_enable(ctrl->dev,
	...
	struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	rc = msm_sensor_get_sub_module_index_sdhi(of_node, &sensordata->sensor_info);
	if (rc < 0) {
		pr_err("%s failed %d\n", __func__, __LINE__);
		goto FREE_SENSORDATA;
	}
	...
	rc = msm_sensor_get_dt_csi_data_sdhi(of_node, &sensordata->csi_lane_params);
	if (rc < 0) {
		pr_err("%s failed %d\n", __func__, __LINE__);
		goto FREE_SENSOR_INFO;
	}

	rc = msm_camera_get_dt_vreg_data_sdhi(of_node,
			&sensordata->power_info.cam_vreg,
			&sensordata->power_info.num_vreg);
	if (rc < 0)
		goto FREE_CSI;

	rc = msm_camera_get_dt_power_setting_data_sdhi(of_node,
			sensordata->power_info.cam_vreg,
			sensordata->power_info.num_vreg,
	...
	if (gpio_array_size) {
		...
		rc = msm_camera_get_dt_gpio_req_tbl_sdhi(of_node, gconf,
		...
		rc = msm_camera_get_dt_gpio_set_tbl_sdhi(of_node, gconf,
			gpio_array, gpio_array_size);
		if (rc < 0) {
			pr_err("%s failed %d\n", __func__, __LINE__);
		...
		rc = msm_camera_init_gpio_pin_tbl_sdhi(of_node, gconf,
			gpio_array, gpio_array_size);
		if (rc < 0) {
			pr_err("%s failed %d\n", __func__, __LINE__);
	...
	rc = msm_sensor_get_dt_actuator_data_sdhi(of_node,
...
int msm_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	return msm_camera_power_down_sdhi(power_info, sensor_device_type,
		sensor_i2c_client);
}
...
int msm_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	rc = msm_camera_power_up_sdhi(power_info, s_ctrl->sensor_device_type,
		sensor_i2c_client);
	if (rc < 0)
		return rc;
	rc = msm_sensor_check_id(s_ctrl);
	if (rc < 0)
		msm_camera_power_down_sdhi(power_info, s_ctrl->sensor_device_type,
					sensor_i2c_client);
